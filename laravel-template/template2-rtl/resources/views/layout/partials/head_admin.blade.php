<meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=0">
        <title>Dreamchat - Dashboard</title>
        
        <!-- Favicon -->
        <link rel="shortcut icon" type="image/x-icon" href="../assets_admin/img/favicon.png">
        <!-- Bootstrap CSS -->
        <link rel="stylesheet" href="../assets_admin/css/bootstrap.rtl.min.css">
      
        <!-- Fontawesome CSS -->
        <link rel="stylesheet" href="../assets_admin/css/font-awesome.min.css">
       <!-- Select2 CSS -->
        <link rel="stylesheet" href="../assets_admin/plugins/select2/css/select2.min.css"> 
        <!-- Datatables CSS -->
        <link rel="stylesheet" href="../assets_admin/plugins/datatables/css/datatables.min.css">
        
        <!-- Feathericon CSS -->
        <link rel="stylesheet" href="../assets_admin/css/feathericon.min.css">
        
        <link rel="stylesheet" href="../assets_admin/plugins/morris/morris.css">

        <!-- VectorMap CSS -->
        <link rel="stylesheet" href="../assets_admin/plugins/vectormap/jqvmap.css">
        
        <!-- Main CSS -->
        <link rel="stylesheet" href="../assets_admin/css/style.css">